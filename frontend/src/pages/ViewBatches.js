...
export default function ViewBatches() {
  ...
  async function allProductBatches() {
    ...
    const url = 'http://127.0.0.1:8082/allproductbatches?manufacturer=' + manufacturer;
    const data = {
      headers: { 
...
function Content({ props, classes, loaded, setLoaded, response, setResponse}) {
  ...
  const recallProduct = async () =>
  {
    // Recall the product
    console.log( props );
    const url = 'http://127.0.0.1:8082/recallProduct?id=' + props.id;
    const data = {
      headers: { 
        'Authorisation': getAuthToken()
      }
    ...
    <Typography variant="h2" component="h2" gutterBottom>
      ...
      <div className='view-products-root'>
        ...
        <div className={classes.buttons}>
          <Button className={ classes.recall } disabled={props.recallStatus} color='primary' onClick={recallProduct} variant='contained'>Recall Product</Button>
